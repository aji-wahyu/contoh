	...
	<script type="text/javascript">
		...
		$("#start,#end").on('change', function() {
			if ($("#start").val() != '' && $("#end").val() != '') {
				...
				var lng = $("#start").find(":selected").attr('lng');
				var lat = $("#start").find(":selected").attr('lat');
				var destinationLng = $("#end").find(":selected").attr('lng');
				var destinationLat = $("#end").find(":selected").attr('lat');
				var url = `https://api.mapbox.com/directions/v5/mapbox/driving-traffic/${lat},${lng};${destinationLat},${destinationLng}?geometries=geojson&steps=true&access_token=${mapboxgl.accessToken}`;

			    fetch(url)
			        .then(response => response.json())
			        .then(data => {
			            var coords = data.routes[0].geometry.coordinates;
			            var distance = data.routes[0].distance * 0.001; 
