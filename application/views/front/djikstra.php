			...
			<div class="page-content">
				<div class="container-fluid">
					<div class="page-content-wrapper">
						...
						<div class="col-12 col-sm-12 mb-5">
								<table id="dataAmbil" class="table table-sm table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
...
<script type="text/javascript">
	...
	var map = new mapboxgl.Map({
			...
			center: [110.1360581, -7.3185605], // posisi awal [lng, lat]
			zoom: 10, // mulai zoom
	...
	var data_obj;
	$.ajax({
		'url': "<?= site_url('admin/graph/ajax/datajoin') ?>",
		'type': 'POST',
		success: function(e) {
			data_obj = JSON.parse(e);
		...
		$('#end').select2();
		var marker = [];
		// MENGAMBIL DATABASE GRAPH
		// KEMUDIAN MENAMPILKAN DALAM BENTUK MARKER BIRU
		$.ajax({
			...
			success: function(e) {
				...
				data_obj.forEach(function(i) {
					if (i.type != '-') {
						var color = i.type == 'object' ? 'blue' : (i.type == '-'? 'red': 'yellow');
						...
						.setPopup(new mapboxgl.Popup().setHTML(`
							...
							<a href="<?= site_url('Wisata/detail/') ?>${i.id}" target="blank" class="badge bg-primary">Lihat detail</a>
							<a href="#" lat="${i.lat}" lng="${i.lng}" onclick="drawRoute(${i.lng},${i.lat})" class="badge bg-success">Rute</a>
							</div>
							</div>
						`)) // tambahkan munculan
						.addTo(map));
		...
		$("#start").change(function(e) {
			...
			if ($("#start").val() == 'current_location') {
				...
				let dataIsiTabel = "<b><tr><td>Awal</td><td>Tujuan</td><td>Jarak</td><td>Waktu</td></tr></b>";
				data_obj.forEach(function(i) {
					if (i.start === idAwal){
						dataIsiTabel += '<tr><td>'+i.name1+'</td><td>'+i.name2+'</td><td>'+i.distance+' Km</td><td>'+(i.time === null ? "" :i.time )+' Menit</td></tr>'
					}
				})

				console.log(dataIsiTabel);
				document.getElementById('dataAmbil').innerHTML = dataIsiTabel;

					// dtb_ = $("#datatable").DataTable({
					// 	"serverSide": true,
					// 	"responsive": true,
					// 	"ajax": {
					// 		"url": "<?= site_url('admin/graph/ajax/listID/') ?>"+idAwal,
					// 		"type": "POST",
					// 	}
					// });
			}
    ...
    function getMatch(e) {
    	var url = 'https://api.mapbox.com/directions/v5/mapbox/driving-traffic/' + e
    ...
    // MEMBUAT ROUTE DARI TITIK AWAL KE LOKASI YANG DI KLIK
    function drawRoute(destinationLng, destinationLat) {
	    var start_atribute = $('#start').select2('data')[0].element.attributes;
	    var lng = start_atribute.lng.value;
	    var lat = start_atribute.lat.value;

	    // Get the start and end coordinates
	    var startCoords = [parseFloat(lat), parseFloat(lng)];
	    var endCoords = [parseFloat(destinationLng), parseFloat(destinationLat)];

	    // Clear existing route
	    removeRoute();
	    
	    // Make a request to the Mapbox Directions API to get turn-by-turn directions
	    var url = `https://api.mapbox.com/directions/v5/mapbox/driving-traffic/${lat},${lng};${destinationLng},${destinationLat}?geometries=geojson&steps=true&access_token=${mapboxgl.accessToken}`;

	    fetch(url)
	        .then(response => response.json())
	        .then(data => {
	            var coords = data.routes[0].geometry.coordinates;
	            var distance = data.routes[0].distance * 0.001; // Convert meters to kilometers
	            var duration = data.routes[0].duration / 60; // Convert seconds to minutes
	            ...
	            // Distance by Turf
	            // distance = turf.distance(turf.point([lat, lng]), turf.point([destinationLng, destinationLat]), {units: 'kilometers'});

	            // Add route details to the map
	            keterangan.innerHTML = '';
	            var routeDetails = `<p>Jarak Rute: ${distance.toFixed(2)} km<br>Durasi Waktu: ${duration.toFixed(2)} minutes</p>`;
	            keterangan.insertAdjacentHTML('beforeend', routeDetails);
	        })
