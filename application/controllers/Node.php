...
class Node extends CI_Controller
{
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('tp', 'Type', 'required', ['required' => '%s tidak boleh kosong ']);
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$_POST['picture'] = '-';
			$this->NodeModel->edit();
			$this->session->set_flashdata('statusMessage', alert('success', 'Data Rumah Sakit berhasil diperbarui'));
			redirect('admin/node');
		}
	...
	public function delete($id)
	{
		...
		$this->session->set_flashdata('statusMessage', alert('success', 'Data Objek Wisata berhasil dihapus'));
		redirect('admin/node');
	}
	...
	public function ajaxlist()
	{
		...
		$datatables->query('SELECT id,name,lat,lng FROM node WHERE type = "-" OR type = "node" ');
		$datatables->hide('id');
